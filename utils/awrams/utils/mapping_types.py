...
class CoordinateSet(ObjectContainer):
    ...
    def update_coord(self,coord):
        self[coord.dimension.name] = coord
        self.dimensions[coord.dimension.name]
        self._set_shape()
...
class Variable:
    ...
    def _dump_attrs(self):
        ...
        attrs['name'] = self.name
        attrs['units'] = self.units.name
        if 'cell_methods' in list(self.meta.keys()):
            attrs['cell_methods'] = self.meta['cell_methods']
...
latitude = Dimension('latitude',deg_north,np.float64)
longitude = Dimension('longitude',deg_east,np.float64)
awrams_time = TimeDimension(dt.date(1900,1,1))
mm = Units('mm')

def extent_to_spatial_coords(extent):
    from .catchments import aquantize

    ref = extent.geospatial_reference()
    lats = np.linspace(ref['lat_origin'],ref['lat_origin'] - (ref['nlats']-1)*ref['cellsize'],ref['nlats'])
    lons = np.linspace(ref['lon_origin'],ref['lon_origin'] + (ref['nlons']-1)*ref['cellsize'],ref['nlons'])
    lats = aquantize(lats,0.05)
    lons = aquantize(lons,0.05)

    return Coordinates(latitude,lats), Coordinates(longitude,lons)
...
def gen_coordset(period,extent):
    ...
    sc = extent_to_spatial_coords(extent)
