...
from awrams.utils.extents import GeospatialReference
...
def geospatial_reference_from_nc(ref_file):
    ref_dict = {}
    lat = ref_file.variables['latitude']
    lon = ref_file.variables['longitude']
    ref_dict['nlats'] = len(lat)
    ref_dict['nlons'] = len(lon)
    ref_dict['lon_origin'] = lon[0]
    ref_dict['lat_origin'] = lat[0]
    ref_dict['mode'] = 'center'
    return GeospatialReference(**ref_dict)
