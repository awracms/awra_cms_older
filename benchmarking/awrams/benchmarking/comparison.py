        ...
        if freq == 'raw':
            ...
            if model is not None:       
                if not model in self.models:
                    ...
                    plot(series=self.models[model].obs[tf][site].resample(_freq),label=self.ref_name,colour='black')
                    plot(series=self.models[model].data[tf][site].resample(_freq),label=self.models[model].name,colour=self.models[model].colour)
            else:
                if freq == 'd':
                    plot(series=self.obs.data[site].resample(_freq),label=self.ref_name,colour='black')
                elif freq == 'm':
                    plot(series=self.obs.monthly[site].resample(_freq),label=self.ref_name,colour='black')
                elif freq == 'y':
                    plot(series=self.obs.annual[site].resample(_freq),label=self.ref_name,colour='black')

                for m in self._iter_models(freq):
                    try:
                        plot(series=m.data[tf][site].resample(_freq),label=m.name,colour=m.colour)
                    except:
