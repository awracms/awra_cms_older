...
def show_shape(shp_file, match=None, ax=None):
    ...
    if match is None:
        ...
        #record = sdb._get_by_field(match.keys()[0],match.values()[0],as_extent=False)
        shapes = []
    ...
    def get_data(idx,extent,sfm):
        if not extent:
            ...
            try:
                ...
                extent = (gb.lon_min,gb.lon_max,gb.lat_min,gb.lat_max) #(111.975,154.025,-44.025,-9.975)
...
def get_cmap(bnd,colors='Blues'):
    ...
    norm = col.BoundaryNorm(bnd, len(bnd))
