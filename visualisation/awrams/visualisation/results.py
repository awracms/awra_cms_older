...
from awrams.utils.extents import Extent,from_georef,default
from awrams.utils.catchments import CatchmentDB
import awrams.utils.datetools as dt
from awrams.utils.io.netcdf_wrapper import start_date,end_date,dataset_frequency,geospatial_reference_from_nc
...
def _infer_time_period(filename,results_folder):
    ...
    start = start_date(Dataset(filenames[0],'r'))
    end = end_date(Dataset(filenames[-1],'r'))
    freq = dataset_frequency(Dataset(filenames[0],'r'))
...
def _infer_extent(filename,results_folder):
    gr = geospatial_reference_from_nc(Dataset(os.path.join(results_folder,filename)))
    return from_georef(gr,compute_areas=False)
...
class ResultsDataSet(TimeSeriesDataSet):
    """
    ...
    def get_data(self,period,extent):
        ...
        if len(period) == 1:
            self._reader.ncd_time_slices = self._locate_period(period)
        else:
