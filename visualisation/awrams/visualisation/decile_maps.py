...
def water_cmap(n=10):
    ...
    '''
    amap = matplotlib.colors.LinearSegmentedColormap.from_list('lsmap',[[1.0,0,0],[1.0,1.0,1.0],[0,0,1.0]],n)
    amap.set_bad((1,1,1))
    amap.set_over((0,0,1))
...
def plot_decile_map(data,pctile_data,pctile_keys,decile_keys,title='',sigma=0.0,scale=1.5):
    ...
    #+++ Hardcoded figsize
    fig = matplotlib.pylab.plt.figure(figsize=(scale*data.shape[1]/100.,scale*data.shape[0]/100.),dpi=100)
    ax = fig.gca()
    ...
    cb2 = matplotlib.colorbar.ColorbarBase(cax,cmap=cmap,norm=norm,ticks=bounds, spacing='proportional') 
...
def plot_decile(data,decile_keys,title='',sigma=0.0,scale=1.5,nmask=None):
    ...
    #lsc.set_bad((0,0,0,1.))
    #lsc._init()
    #lsc._lut[:,-1] = np.linspace(0.0,1.0,lsc.N+3)[::-1]
